#!/usr/bin/env node
// SceneF — movie showtimes across California, Florida and Hawaii, as a LOCAL MCP
// server over stdio.
//
...
export const INSTRUCTIONS = [
  "Read-only movie showtimes across 56 regional boards in California, Florida and Hawaii — chains, independents, repertory and drive-ins. Board-reading tools take a `region`; omitted means the default board, and it is never inferred from where you are. scenef_resolve_board takes a place to find the appropriate board. Call scenef_now for the list of boards — each id comes with the place it names, so sgv reads as the San Gabriel Valley rather than a bare handle. TIMES ARE THE THEATRE'S OWN WALL CLOCK, not one city's — Hawaii is three hours off Pacific and keeps no DST, so a board's timezone is reported with its data and is the only zone its times mean anything in. A screening belongs to the NIGHT it opens: a 12:15am Saturday show is Friday night. Use ticket and calendar URLs exactly as returned, preserving their region query parameters. Ticket links use https://scenef.com/go/{screeningId}; film pages https://scenef.com/film/{slug}, or /film/{slug}/{regionSlug} for a specific board. Start with scenef_now to check freshness, scenef_plan_movie_night for recommendations.",
  ACCURACY_CONTRACT,
  "Call scenef_accuracy for that record as numbers you can quote. Every returned showtime carries confidence, source_tier, sources, verified_at and freshness. Use freshness to distinguish retained or stale source evidence from a newly generated board; detailed responses include more rows, not stronger evidence. Chain showtimes (AMC, Regal, Apple Cinemas), when present, are licensed from an aggregator rather than read from the theater — they are labeled source_tier licensed-feed and carry the weakest claim on the site.",
].join(" ");
